...
# --- DEFINIZIONE DEI PROTOCOLLI QUANTISTICI ---
    ...
    def run(self):
        for i in range(self.num_bits):
            ...
            if self.noise_probability > 0:
                if random.random() < self.noise_probability: qapi.operate(qubit_a, ns.Z)
                if random.random() < self.noise_probability: qapi.operate(qubit_b, ns.Z)
